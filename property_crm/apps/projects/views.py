...
This gives you:
    ...
    GET  /api/v1/projects/{id}/commit-feasibility/        → commit active scenario to project
    GET  /api/v1/development-applications/{id}/documents/ → list documents for a DA
    POST /api/v1/development-applications/{id}/documents/ → upload a document to a DA
    DELETE /api/v1/da-documents/{id}/                   → delete a DA document
"""
...
class ProjectViewSet(
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Feasibility commit endpoint
    # ─────────────────────────────────────────────────────────────────────────

    @action(
        detail=True,
        methods=["post"],
        url_path="commit-feasibility",
        permission_classes=[HasPermission("da.manage")],
    )
    def commit_feasibility(self, request, pk=None):
        """
        POST /api/v1/projects/{id}/commit-feasibility/

        Commits the active feasibility scenario to the project.
        Writes total_lots → billing_lot_count and total_gr → target_gr.
        Marks feasibility_committed = True.
        """
        from django.utils import timezone

        project = self.get_object()
        scenarios = (project.feasibility or {}).get("scenarios", [])

        if not scenarios:
            return Response({"detail": "No feasibility scenarios found."}, status=status.HTTP_400_BAD_REQUEST)

        active = next((s for s in scenarios if s.get("is_active")), None)
        if not active:
            return Response({"detail": "No active scenario set."}, status=status.HTTP_400_BAD_REQUEST)

        lines = active.get("lines", [])
        if not lines:
            return Response({"detail": "Active scenario has no lot type lines."}, status=status.HTTP_400_BAD_REQUEST)

        # Calculate totals from active scenario
        total_lots = 0
        total_gr   = 0
        for line in lines:
        ...
        project.billing_lot_count        = total_lots
        project.target_gr                = round(total_gr, 2)
        project.feasibility_committed    = True
        project.feasibility_committed_at = timezone.now()
        project.save(update_fields=[
            "billing_lot_count", "target_gr",
            "feasibility_committed", "feasibility_committed_at",
        ])
...
class DADocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = DADocument
        fields = [
            "id", "da", "category", "category_display",
            "title", "file_url", "filename",
...
class DevelopmentApplicationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs  = DevelopmentApplication.objects.filter(
            project__organisation=org
        ).select_related("project", "stage").prefetch_related("conditions", "milestones", "documents")
    ...
    def documents(self, request, pk=None):
        """
        GET  /api/v1/development-applications/{id}/documents/
        POST /api/v1/development-applications/{id}/documents/

        POST body (multipart/form-data):
            category    decision_notice | approved_plans | condition_schedule |
                        referral_response | correspondence | other
        ...
        if request.method == "GET":
            docs = da.documents.all()
            serializer = DADocumentSerializer(
                docs, many=True, context={"request": request}
            )
            return Response(serializer.data)

        # POST — upload
        serializer = DADocumentUploadSerializer(
            data=request.data, context={"request": request}
        )
        serializer.is_valid(raise_exception=True)
        serializer.save(da=da, uploaded_by=request.user)
        return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class DADocumentViewSet(
    mixins.DestroyModelMixin,
    viewsets.GenericViewSet,
):
    """
    DELETE /api/v1/da-documents/{id}/

    Upload is via POST /api/v1/development-applications/{id}/documents/
    List  is via GET  /api/v1/development-applications/{id}/documents/
    This viewset exists solely for the delete action.
    """
