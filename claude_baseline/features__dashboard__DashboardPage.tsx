...
const ACTIVITY_ICON: Record<string, { icon: string; color: string; bg: string }> = {
  call:       { icon: 'ti-phone',    color: '#2649a0', bg: '#eef2fb' },
  email:      { icon: 'ti-mail',     color: '#5b2d8a', bg: '#f5f0fb' },
  meeting:    { icon: 'ti-users',    color: '#2649a0', bg: '#eef2fb' },
  inspection: { icon: 'ti-building', color: '#237a3d', bg: '#eef7f0' },
  task:       { icon: 'ti-checkbox', color: '#9a5f00', bg: '#fef6ec' },
  note:       { icon: 'ti-notes',    color: '#7a6e68', bg: '#f2f0ee' },
}

const PIPELINE_STAGES = [
  { key: 'on_hold',         label: 'On hold',         color: '#f0c87a' },
  { key: 'pending',         label: 'Pending',         color: '#d4920e' },
  { key: 'reserved',        label: 'Reserved',        color: '#8fb3e8' },
  { key: 'contract_issued', label: 'Contract issued', color: '#4a72c4' },
  { key: 'exchanged',       label: 'Exchanged',       color: '#2649a0' },
  { key: 'settled',         label: 'Settled',         color: '#8a8280' },
]
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{
      ...
      padding: '10px 14px', boxShadow: '0 4px 12px rgba(44,36,32,0.10)', fontSize: 12,
    }}>
      <div style={{ fontWeight: 600, color: '#2c2420', marginBottom: 6 }}>{label}</div>
      {payload.map((p: any) => (
        <div key={p.dataKey} style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#5a4e48', marginBottom: 2 }}>
  ...
  return (
    <div style={{ ...CARD_STYLE, padding: '16px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13, fontWeight: 500, color: '#2c2420' }}>
          <TrendingUp size={15} color="#7a6e68" />
          Sales over time
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          {(['daily', 'weekly'] as ChartPeriod[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)} style={{
              ...
              background: period === p ? '#3d4a5c' : '#f2f0ee',
              color: period === p ? '#fff' : '#7a6e68',
      ...
      {!hasData ? (
        <div style={{ height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a89e98', fontSize: 13 }}>
        ...
        <ResponsiveContainer width="100%" height={180}>
          ...
          >
            ...
            <YAxis
              ...
              axisLine={false}
            />
            <Tooltip content={<ChartTooltip />} cursor={{ fill: 'rgba(44,36,32,0.03)' }} />
            <Legend
              iconType="square"
              iconSize={8}
              wrapperStyle={{ fontSize: 11, color: '#7a6e68', paddingTop: 8 }}
            />
            <Bar dataKey="reserved"  name="Reserved"  fill="#8fb3e8" radius={[3,3,0,0]} />
            <Bar dataKey="exchanged" name="Exchanged" fill="#2649a0" radius={[3,3,0,0]} />
            <Bar dataKey="settled"   name="Settled"   fill="#52a96e" radius={[3,3,0,0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: '16px 20px', borderBottom: '0.5px solid #e8e2dd', flexShrink: 0,
        }}>
          <p style={{ fontSize: 14, fontWeight: 500, color: '#2c2420' }}>{title}</p>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, color: '#7a6e68' }}>
...
function MetricCard({ label, value, sub, valueColor, icon, alert }: {
  ...
  return (
    <div style={{
      ...CARD_STYLE,
      padding: '14px 16px',
      background: alert ? '#fef6ec' : '#ffffff',
      border: alert ? '1px solid #fcd9a0' : '1px solid #e8e2dd',
      boxShadow: alert ? '0 1px 3px rgba(192,83,58,0.10), 0 4px 12px rgba(192,83,58,0.07)' : CARD_STYLE.boxShadow,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
        <span style={{ color: '#7a6e68', display: 'flex' }}>{icon}</span>
        <span style={{ fontSize: 12, color: '#7a6e68' }}>{label}</span>
      </div>
      <div style={{ fontSize: 22, fontWeight: 500, color: valueColor ?? '#2c2420' }}>{value}</div>
      {sub && <div style={{ fontSize: 11, color: '#a89e98', marginTop: 3 }}>{sub}</div>}
    </div>
  )
}
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
      ...
      {action && (
        <button onClick={onAction} style={{
          ...
          fontSize: 11, color: '#7a6e68', display: 'flex', alignItems: 'center', gap: 3, padding: 0,
        }}>
...
function Divider() {
  return <div style={{ height: '0.5px', background: '#f0ebe6', margin: '0 -20px' }} />
}
...
function Initials({ text, size = 28 }: { text: string; size?: number }) {
  return (
    <div style={{
      ...
      background: '#f7ece9', color: '#c0533a',
...
export default function DashboardPage() {
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {[1,2,3].map(i => <div key={i} style={{ height: 80, borderRadius: 12, background: '#f0ebe6' }} />)}
      </div>
    )
  }
  ...
  if (isError || !data) {
    return (
      <div style={{ padding: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, color: '#a89e98' }}>
        <AlertCircle size={32} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f6f4' }}>
      ...
      <div style={{
        ...
        padding: '20px 24px 18px', boxShadow: '0 1px 2px rgba(44,36,32,0.04)',
      }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 500, color: '#2c2420' }}>
              {greeting}, {user?.first_name}
            </h1>
            <p style={{ fontSize: 13, color: '#7a6e68', marginTop: 3 }}>
              Here's what's happening across your portfolio
            </p>
          </div>
          <div style={{
            fontSize: 12, color: '#7a6e68', background: '#f9f6f4',
      ...
      <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 10 }}>
          ...
          <MetricCard
            ...
            valueColor={m.active_sales > 0 ? '#9a5f00' : '#2c2420'}
          ...
          <MetricCard
            ...
            sub="within 4 hours"
            valueColor={m.expiring_soon > 0 ? '#882010' : '#2c2420'}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <SectionCard>
            ...
            {totalPipeline > 0 ? (
              <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 12px', marginBottom: 14 }}>
                  {PIPELINE_STAGES.map(({ key, label, color }) => {
                    const count = data.pipeline[key] ?? 0
                    return count > 0 ? (
                      <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#7a6e68' }}>
              ...
              <p style={{ fontSize: 12, color: '#a89e98', marginBottom: 14 }}>No active sales.</p>
            )}
            ...
            {data.pending_approvals.length > 0 && (
              <>
                ...
                <div style={{ marginTop: 12 }}>
                  <p style={{ fontSize: 11, fontWeight: 500, color: '#a89e98', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
                    Pending approval
                  </p>
                  {data.pending_approvals.map(sale => (
                    <div key={sale.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '0.5px solid #f0ebe6' }}>
                      <Initials text={sale.buyer_initials} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 12, fontWeight: 500, color: '#2c2420', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {sale.buyer_name}
                        </p>
                        <p style={{ fontSize: 11, color: '#7a6e68' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: '#eef7f0', color: '#1a5c2e', border: '0.5px solid #b8dfc3',
          ...
          <SectionCard>
            ...
            {data.recent_activities.length === 0 ? (
              <p style={{ fontSize: 12, color: '#a89e98' }}>No recent activity.</p>
            ) : (
              data.recent_activities.map((a, i) => {
                ...
                return (
                  <div key={a.id} style={{
                    ...
                    borderBottom: i < data.recent_activities.length - 1 ? '0.5px solid #f0ebe6' : 'none',
                  }}>
                    <div style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {a.assigned_to_name && <p style={{ fontSize: 11, color: '#7a6e68', marginTop: 1 }}>{a.assigned_to_name}</p>}
                    </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
          ...
          <SectionCard>
            ...
            {data.projects.length === 0 ? (
              <p style={{ fontSize: 12, color: '#a89e98' }}>No active projects.</p>
            ) : (
              data.projects.map((p, i) => (
                <div key={p.id} onClick={() => navigate(`/projects/${p.id}`)} style={{
                  display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', cursor: 'pointer',
                  borderBottom: i < data.projects.length - 1 ? '0.5px solid #f0ebe6' : 'none',
                }}>
                  <div style={{ width: 34, height: 34, borderRadius: 8, background: '#f2f0ee', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Building2 size={16} color="#a89e98" />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 12, fontWeight: 500, color: '#2c2420' }}>{p.name}</p>
                    <p style={{ fontSize: 11, color: '#7a6e68' }}>{p.total_lots} lots · {p.stage_count} stage{p.stage_count !== 1 ? 's' : ''}</p>
                    <div style={{ height: 4, background: '#f0ebe6', borderRadius: 2, marginTop: 5, overflow: 'hidden' }}>
                      <div style={{ height: '100%', borderRadius: 2, background: '#4a72c4', width: `${Math.round((p.settled_lots / Math.max(p.total_lots, 1)) * 100)}%` }} />
                    </div>
                  </div>
                  <ChevronRight size={14} color="#d4ccc5" />
                </div>
              ))
            )}
          </SectionCard>
          ...
          <SectionCard>
            ...
            {data.my_tasks.length === 0 ? (
              <p style={{ fontSize: 12, color: '#a89e98' }}>No open tasks.</p>
            ) : (
              data.my_tasks.map((t, i) => {
                const { label, overdue } = formatDueDate(t.due_date)
                return (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: i < data.my_tasks.length - 1 ? '0.5px solid #f0ebe6' : 'none' }}>
                    <div style={{ width: 16, height: 16, borderRadius: '50%', border: '1.5px solid #d4ccc5', flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, color: '#2c2420', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.subject}</p>
                      {t.lot_number && <p style={{ fontSize: 11, color: '#a89e98' }}>Lot {t.lot_number} · {t.project_name}</p>}
                    </div>
                    <span style={{ fontSize: 11, color: overdue ? '#882010' : '#a89e98', whiteSpace: 'nowrap', flexShrink: 0 }}>{label}</span>
                  </div>
                )
              })
            )}
          ...
          <SectionCard>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { label: 'New sales',       value: `+${data.this_month.new_sales}`,              color: '#237a3d' },
                { label: 'Fallen over',     value: data.this_month.fallen_over,                  color: data.this_month.fallen_over > 0 ? '#882010' : '#2c2420' },
                { label: 'Settled',         value: data.this_month.settled,                      color: '#2c2420' },
                { label: 'Revenue settled', value: formatPrice(data.this_month.revenue_settled), color: '#2649a0' },
              ].map(({ label, value, color }) => (
                <div key={label} style={{
                  display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                  padding: '8px 10px', background: '#f9f6f4', borderRadius: 8, border: '1px solid #f0ebe6',
                }}>
                  <span style={{ fontSize: 12, color: '#7a6e68' }}>{label}</span>
      ...
      <SlidePanel open={panel !== null} onClose={() => setPanel(null)} title="Details">
        <p style={{ fontSize: 13, color: '#7a6e68' }}>Detail view coming soon.</p>
      </SlidePanel>
    </div>
  )
}