...
function formatDueDate(iso: string | null): { label: string; overdue: boolean } {
  ...
  return {
    label: due.toLocaleDateString('en-AU', { weekday: 'short', day: 'numeric', month: 'short' }),
    overdue: false,
  }
}
...
function MetricCard({
  label, value, sub, valueColor, icon, alert,
}: {
  label: string
  value: string | number
  sub?: string
  ...
  return (
    <div style={{
      background: alert ? '#FFF7ED' : '#f9fafb',
      borderRadius: 8,
      padding: '14px 16px',
      border: alert ? '0.5px solid #FED7AA' : '0.5px solid transparent',
    }}>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
      ...
      {action && (
        <button
          onClick={onAction}
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 11, color: '#6b7280', display: 'flex', alignItems: 'center', gap: 3, padding: 0 }}
        >
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 10 }}>
          ...
          <MetricCard
            ...
            value={`+${data.this_month.new_sales}`}
            sub={`${data.this_month.fallen_over} fallen over`}
            valueColor="#0F6E56"
            icon={<TrendingUp size={14} />}
          />
          <MetricCard
            label="On hold expiring"
              ...
              <>
                <div style={{ display: 'flex', gap: 3, height: 8, borderRadius: 4, overflow: 'hidden', marginBottom: 10 }}>
                  {PIPELINE_STAGES.map(({ key, color }) => {
                    ...
                    return (
                      <div key={key} style={{ flex: count, background: color, minWidth: 4 }} />
                    )
                  })}
                </div>
          ...
          <SectionCard>
            ...
            {data.recent_activities.length === 0 ? (
              ...
              data.recent_activities.map((a, i) => {
                const style = ACTIVITY_ICON[a.activity_type] ?? ACTIVITY_ICON.note
                return (
                  <div key={a.id} style={{
              ...
              data.projects.map((p, i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: '#6b7280' }}>
                      {p.total_lots} lots · {p.stage_count} stage{p.stage_count !== 1 ? 's' : ''}
                    </p>
                    <div style={{ height: 4, background: '#f3f4f6', borderRadius: 2, marginTop: 5, overflow: 'hidden' }}>
                      <div style={{
                        ...
                        width: `${Math.round((p.settled_lots / Math.max(p.total_lots, 1)) * 100)}%`,
                      }} />
                    </div>
                  </div>
              ...
              data.my_tasks.map((t, i) => {
                ...
                return (
                  <div key={t.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, color: '#111827', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {t.subject}
                      </p>
                      {t.lot_number && (
                        <p style={{ fontSize: 11, color: '#9ca3af' }}>Lot {t.lot_number} · {t.project_name}</p>
                      )}
                    </div>
                    <span style={{ fontSize: 11, color: overdue ? '#B91C1C' : '#9ca3af', whiteSpace: 'nowrap', flexShrink: 0 }}>
                      {label}
                    </span>
                  </div>
                )
      ...
      {/* Slide panel (reserved for future drill-downs) */}
